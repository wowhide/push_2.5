...
class AndroidPushNotifier {
    public function push() {
        try {
            ...
            // 今日配信のお知らせが存在するかチェック
            $noticeSQL = "SELECT * FROM t_notice_info WHERE notice_schedule = '" . date("Ymd", time()) . "'";
            ...
            foreach ($noticeList as $notice) {
                ...
                if($notice['search_category'] == 4 || $notice['search_category'] == 5) {
                    $getRegistraSQL = "SELECT
                                       ...
                                       WHERE
                                           allow_push = 1 AND
                                           deceased_id IN (SELECT
                                                               deceased_id
                                                           FROM
                                                               t_notice_target
                                                           WHERE
                                                               notice_info_no = " . $notice['notice_info_no'] . ")";
                } else {
                    $getRegistraSQL = "SELECT DISTINCT registration_id
                                       FROM
                                           c_android_registration_id
                                       WHERE
                                           allow_push = 1 AND
